...
app.post("/send-email", async (req, res) => {
    ...
    try {
      ...
      invitedEmail = email;  // Store the email of the user being invited
...
app.post("/ai-conversation", async (req, res) => {
    ...
    // Use the stored email from the invitation process
    ...
    try {
      ...
      const apiRequest = {
        prompt: message,  // Use the user-provided message as the prompt
        max_tokens: 150,
      };
      ...
      const mailOptions = {
        from: EMAIL_USER,  // Sender's email (owner's email)
        to: email,         // Recipient's email (the user who asked the question)
